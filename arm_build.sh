  ...
  case "$BUILD_OPTION" in
    ...
    all)
      case "$CHIP_FAMILY" in
        rockchip)
          ...
          ;;
        sunxi)
          if [ -x "$BASE_DIR/sunxi-compile.sh" ]; then
            "$BASE_DIR/sunxi-compile.sh" all
          else
            error "sunxi-compile.sh not found or not executable"
          fi
          ;;
        *)
    ...
    *)
      case "$CHIP_FAMILY" in
        ...
        sunxi)
          if [ -x "$BASE_DIR/sunxi-compile.sh" ]; then
            "$BASE_DIR/sunxi-compile.sh" "$BUILD_OPTION"
          else
            error "sunxi-compile.sh not found or not executable"
          fi
          ;;
